...
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
	WHERE dept_id = 'SYS' AND hire_date >= '2018-01-01';
...
SELECT emp_id, emp_name, dept_id, gender, hire_date, email, salary
	FROM employee
    WHERE retire_date IS NULL
...
SELECT emp_name, emp_id, hire_date, salary,
	   IF(salary IS NULL, NULL, IF(salary >= 7000, '상', IF(salary >= 5000, '중', '하'))) AS salary_lvl
	FROM employee
    WHERE retire_date IS NULL;